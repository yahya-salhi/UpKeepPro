  ...
  useEffect(() => {
    if (isOpen && tool && historyData) {
      ...
      const entries = historyData
        .filter(entry => entry.eventType === "entry" && entry.reference?.startsWith("pv-"))
        .map(entry => ({
      ...
      <DialogContent className="sm:max-w-[500px] bg-white rounded-lg shadow-xl">
        <DialogHeader className="border-b pb-4">
          ...
          <p className="text-sm text-gray-500">
            Tool: <span className="font-medium">{tool.designation}</span> ({tool.mat})
          </p>
        </DialogHeader>
        ...
        {isLoadingHistory ? (
          <div className="py-12 flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-muted-foreground">Loading tool history...</p>
          </div>
        ) : pvEntries.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              This tool doesn&apos;t have any PV entries in its history that can be converted to M11.
            </p>
            <Button 
              type="button" 
              variant="outline" 
          ...
          {pvEntries.length === 1 ? (
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800 mb-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-medium text-blue-800 dark:text-blue-300">Single PV Entry Found</h3>
                  <p className="text-sm text-blue-600 dark:text-blue-400 mt-1">
                    Converting PV entry: <span className="font-medium">{pvEntries[0].reference}</span> from {pvEntries[0].date}
                  </p>
                  <div className="mt-2 flex items-center gap-2">
                    <span className="text-sm font-medium text-blue-600 dark:text-blue-400">Quantity: +{pvEntries[0].quantity}</span>
                    {pvEntries[0].isMainAcquisition && (
                      <Badge variant="outline" className="text-xs bg-blue-100 text-blue-700 border-blue-200">Main Acquisition</Badge>
                    )}
                  </div>
                </div>
              </div>
              <input type="hidden" {...register("conversionMode")} value="specific" />
            </div>
          ) : (
            <>
              ...
              <div className="space-y-3">
                ...
                <RadioGroup 
                  ...
                  onValueChange={(value) => {
                    ...
                    register("conversionMode").onChange({ target: { name: "conversionMode", value } });
                  }}
                >
                  <div className="flex items-center space-x-2 rounded-md border p-3 hover:bg-muted/50">
                    ...
                    <Label htmlFor="specific" className="flex-1 cursor-pointer">
                      <div className="font-medium">Convert Specific PV Entry</div>
                      <div className="text-sm text-muted-foreground">
                        Convert an individual PV entry to M11 while keeping others as PV
                      </div>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2 rounded-md border p-3 hover:bg-muted/50">
                    ...
                    <Label htmlFor="all" className="flex-1 cursor-pointer">
                      <div className="font-medium">Convert All PV Entries</div>
                      <div className="text-sm text-muted-foreground">
                ...
                <input type="hidden" {...register("conversionMode")} value={conversionMode} />
              </div>
                        ...
                        >
                          <div className="flex justify-between items-center">
                            <div>
                              ...
                              <div className="text-sm text-muted-foreground">Date: {entry.date}</div>
                            </div>
              ...
              {conversionMode === "all" && (
                <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-100 dark:border-green-800">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="font-medium text-green-800 dark:text-green-300">Converting All PV Entries</h3>
                      <p className="text-sm text-green-600 dark:text-green-400 mt-1">
                        All {pvEntries.length} PV entries will be converted to the same M11 reference.
                      </p>
                      <div className="mt-3 text-xs text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-800/50 p-2 rounded">
                        <ul className="list-disc list-inside space-y-1">
                          {pvEntries.slice(0, 3).map((entry, index) => (
                            <li key={index}>{entry.reference} ({entry.date})</li>
                          ))}
                          {pvEntries.length > 3 && (
                            <li>And {pvEntries.length - 3} more entries...</li>
                          )}
                        </ul>
                      </div>
                    </div>
